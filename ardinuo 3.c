...
#include <NewPing.h>
#include <TimeLib.h>
#include <Pushbullet.h> // Include the Pushbullet library
...
const char* pushbulletToken = "your_pushbullet_api_token";
Pushbullet pushbullet(pushbulletToken);
void setup() {
Serial.begin(115200);
...
// Send notification to user about low water level
 pushbullet.sendNote("Water Level Alert", "The water level is low. Please refill the fountain.");
}
}
// Indicate water level using the LED (for example, turn on if level is below 10 cm)
int distance = sonar.ping_cm();
 if (distance < 10) {
digitalWrite(ledPin, HIGH);
 } else {
